...
puts meeting_id
puts playback
if (playback == "slides-enhanced")
	logger = Logger.new("/var/log/bigbluebutton/slides-enhanced/publish-#{meeting_id}.log", 'daily' )
	BigBlueButton.logger = logger
        BigBlueButton.logger.info("Publishing #{meeting_id}")
	# This script lives in scripts/archive/steps while properties.yaml lives in scripts/
	bbb_props = YAML::load(File.open('../../core/scripts/bigbluebutton.yml'))
	simple_props = YAML::load(File.open('slides-enhanced.yml'))
	
	recording_dir = bbb_props['recording_dir']
	process_dir = "#{recording_dir}/process/slides-enhanced/#{meeting_id}"
	publish_dir = simple_props['publish_dir']
	playback_host = bbb_props['playback_host']
	
	target_dir = "#{recording_dir}/publish/slides-enhanced/#{meeting_id}"
	if not FileTest.directory?(target_dir)
		...
		metaxml = b.recording {
		  ...
		  b.playback {
		  	b.format("slides-enhanced")
		  	b.link("http://#{playback_host}/playback/slides-enhanced/playback.html?meetingId=#{meeting_id}")
	  	}
		  b.meta {
    ...
    #Create slides.xml
    #presentation_url = "http://" + playback_host + "/slides-enhanced/" + meeting_id + "/presentation"
    presentation_url = "/slides-enhanced/" + meeting_id + "/presentation"
